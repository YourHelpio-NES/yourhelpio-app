...
export default function ThemeDetailsContent({ activeRow }: { activeRow: ThemeTableRow }) {
  return (
      ...
      >
        ...
        >
          <span className="d-flex gap-2 align-items-start">
            ...
            <CardTitle>Тема «{activeRow.themeName}»</CardTitle>
          </span>
          ...
          <LabelSection label="Результати навчання:">
            <BasicBlock $bgColor="transparent">
              {learningOutcomesTheme.map((item, index) => {
                const status =
                  index === 0
                    ? DifficultyEnum.HIGH
                    : index === 1 || index === 2
          ...
          <LabelSection label="Ключові терміни:">
            <BasicBlock className="flex-wrap" $direction={'row'} $gap="10px" $bgColor="transparent">
              {keywordsTheme.map((item, index) => (
                <Button
                  type="small"
                  width="auto"
